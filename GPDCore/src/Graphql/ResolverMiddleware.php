...
class ResolverMiddleware
{
     ...
     * 
     * Un middleware es un callable que recibe el resolver original y retorna uno nuevo
     * que puede ejecutar lógica antes/después del resolver original.
     *
     * @param callable $resolver Resolver original
     * @param callable|null $middleware Función middleware que envuelve al resolver
     ...
     */
    public static function wrap(callable $resolver, ?callable $middleware): callable
    {
        if ($middleware === null) {
            return $resolver;
        }
     ...
     *
     * @param callable $resolver Resolver original
     * @param array<callable> $middlewares Array de funciones middleware
     * @return callable Resolver con todos los middlewares encadenados
     * @throws InvalidArgumentException Si algún elemento del array no es callable
     * 
     * @example
    ...
    {
        // Validar que todos los elementos sean callables
        foreach ($middlewares as $index => $middleware) {
            if (!is_callable($middleware)) {
                throw new InvalidArgumentException(
                    "Middleware at index {$index} is not callable"
                );
            }
        }
